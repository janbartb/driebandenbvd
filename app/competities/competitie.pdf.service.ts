...
export class CompetitiePdfService {
    ...
    makePdfCompetitieStand(competitie: ICompetitie): void {
        ...
        // vertical fat
        var osTsCar: number = 84.5;
        var osTsGem: number = 104;
        var osRangP: number = 119.5;
        var osRangR: number = 135;
        var osPunt: number = 150;
        var osRend: number = 170;
        var osWed: number = 185;
        var osTotC: number = 205;
        var osTotB: number = 221;
        var osTotG: number = 240;
        var osHSer: number = 257;
        var osHGem: number = 277;
        var aantSpl: number = (competitie.spelers) ? competitie.spelers.length : 0;
        var lineHeight: number = 8;
        var len: number = 12 + (lineHeight * aantSpl);
        ...
        var osVertStart: number = 44;
        var self = this;
        ovzData.forEach(function (speler: ICompStand) {
    ...
    private getPosition(txt: string, offset: number): number {
        let len = txt.length * this.lenChar;
        if (txt.indexOf('.') >= 0) {
    ...
    private rondAf(getal: number, aantDec: number): string {
        let factor: number = (aantDec == 2) ? 100 : 1000;
        let val: number = getal * factor;
        return (Math.round(val) / factor).toFixed(aantDec);
    }

    private getNewPdf(orientatie: string, uom: string, paperSize: string): any {
